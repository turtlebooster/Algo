...
public class SW_D3_4698_테네스의특별한소수 {
	public static void main(String[] args) {
		...
		// 시간초과가 나오는 관계로... 소수리스트를 미리 만듦
		// 1 이상 1000000 이하의 정수 중에서 체크
		// 1은 소수 아님 -> 2부터
		for (int n = 2; n <= 1000003; n++) {
			boolean flag = true; // 소수인지 체크
			// 1 과 자기자신 제외 -> 2 부터 체크
			for (int i = 2, R = n / 2; i <= R ; R = n / ++i) {
				if (n != i * R) continue; 			// 둘의 곱이 n 이 되지 않으면 i는 약수가 아님
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			for (int i = 0; sosu.get(i) <= B;i++) {
				if ( A > sosu.get(i)) continue;
				if (!(sosu.get(i) + "").contains(D + "")) continue;
				ans++;
			}
