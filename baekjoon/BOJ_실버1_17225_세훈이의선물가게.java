...
public class BOJ_실버1_17225_세훈이의선물가게 {
	public static void main(String[] args) {
		...
		for (int i = 1; i <= N; i++) {
			...
			total += m;
			if ("B".equals(c)) Border.put(t, m);
			else Rorder.put(t, m);
		}
		
		int time = 0;
		int giftNo = 0;
		int Btopack = 0, Rtopack = 0;
		int Bst = -Bt, Rst = -Rt;
		while (giftNo < total) {
			// B 주문 받음 -> B 포장해야하는 선물 수 추가
			if (Border.get(time) != null) Btopack += Border.get(time);				
			// R 주문 받음 -> R 포장해야하는 선물 수 추가
			if (Rorder.get(time) != null) Rtopack += Rorder.get(time);
